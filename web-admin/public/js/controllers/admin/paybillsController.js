angular.module('PaybillsCtrl',[]).controller('PaybillsController',function($scope,$mdDialog,CarwashVars,PaybillServ){
	...
	function ReloadData(){
		...
		if(CarwashActive == 0){
			...
			PaybillServ.AllByAccount(Profile.id,initialDate,finalDate).success(function(data){
				if(data.success){
					self.PaybillsOnDB = data.paybills;
					self.CountPaybills = self.PaybillsOnDB.length;
					var total = 0;
					angular.forEach(self.PaybillsOnDB, function(paybill, key) {
						total = total + paybill.total;
						paybill.date = moment(paybill.date).format('YYYY-MM-DD');
					});
					self.TotalPaybills = total;
				}else{
			...
			//byCarwash
			PaybillServ.AllByLavado(CarwashActive,initialDate,finalDate).success(function(data){
				if(data.success){
					self.PaybillsOnDB = data.paybills;
					self.CountPaybills = self.PaybillsOnDB.length;
					var total = 0;
