angular.module('IngressCtrl',[]).controller('IngressController',function($scope,$mdDialog,CarwashVars,IngressServ){
	...
	function ReloadData(){
		...
		if(CarwashActive == 0){
			...
			IngressServ.AllByAccount(Profile.id,initialDate,finalDate).success(function(data){
				if(data.success){
					self.IngressesOnDB = data.ingresses;
					self.CountIngresses = self.IngressesOnDB.length;
					var total = 0;
					angular.forEach(self.IngressesOnDB, function(ingress, key) {
						total = total + ingress.total;
						ingress.date = moment(ingress.date).format('YYYY-MM-DD');
					});
					self.TotalIngresses = total;
				}else{
			...
			//byCarwash
			IngressServ.AllByLavado(CarwashActive,initialDate,finalDate).success(function(data){
				if(data.success){
					self.IngressesOnDB = data.ingresses;
					self.CountIngresses = self.IngressesOnDB.length;
					var total = 0;
