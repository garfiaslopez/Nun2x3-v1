angular.module('StockCtrl',[]).controller('StockController',function($scope,$mdDialog,CarwashVars,StockServ){
	...
	function ReloadData(){
		...
		if(CarwashActive == 0){
			...
			StockServ.AllByAccount(Profile.id,initialDate,finalDate).success(function(data){
				if(data.success){
					self.StocksOnDB = data.stocks;
					self.CountStocks = self.StocksOnDB.length;
					var total = 0;
					angular.forEach(self.StocksOnDB, function(stock, key) {
						total = total + stock.total;
						stock.deliverDate = moment(stock.deliverDate).format('YYYY-MM-DD');
					});
					self.TotalStocks = total;
				}else{
			...
			//byCarwash
			StockServ.AllByLavado(CarwashActive,initialDate,finalDate).success(function(data){
				if(data.success){
					self.StocksOnDB = data.stocks;
					self.CountStocks = self.StocksOnDB.length;
					var total = 0;
